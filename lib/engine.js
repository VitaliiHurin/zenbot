      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            msg(`price = n(mid_price).subtract(n(mid_price).multiply(so.markup_pct / 100)).format(s.product.increment, Math.floor)\nprice = n(${mid_price}).subtract(n(${mid_price}).multiply(${so.markup_pct} / 100)).format(${s.product.increment}, Math.floor) = ${price}`)
            if (!size) {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct && !s.acted_on_stop) {
                ...
                if (s.balance.asset_hold != 0 && n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
