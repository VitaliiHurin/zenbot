  ...
  return function (s) {
      ...
      if (so.mode !== 'live') {
        ...
        s.exchange[type](order_copy, function (err, api_order) {
          ...
          if (api_order.status === 'done') {
            msg(type + ' order placed and fully accepted immediately at ' + fc(order.price))
            executeOrder(order)
            ...
            s.exchange.cancelOrder({order_id: order.order_id}, function (err) {
              function checkHold (try_n) {
                s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
                  ...
                  if (err || api_order.error) {
                    return cb(null, do_reorder ? null : false)
                  }
                  if (api_order) {
                    s.api_order = api_order
                  ...
                  syncBalance(function () {
                    ...
                    if (on_hold) {
                      ...
                      setTimeout(checkHold, c.wait_for_settlement, try_n)
                    }
                    else {
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                getQuote(function (err, quote) {
                  ...
                  if (type === 'buy') {
                    ...
                    msg('order.price [' + n(order.price).value() + '] < marked_price[' + marked_price + '] ???')
                    //if (
                    //  (so.markup_pct >= 0 && n(order.price).value() < marked_price) ||
                    //  (so.markup_pct < 0 && n(order.price).value() > marked_price)
                    //) {
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      if (!is_reorder && s[signal + '_order']) {
        ...
        msg('executeSignal - order already placed')
        _cb && _cb(null, null)
        return
      }
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            //msg(`price = n(quote.bid).subtract(n(quote.bid).multiply(so.markup_pct / 100)).format(s.product.increment, Math.floor)\nprice = n(${quote.bid}).subtract(n(${quote.bid}).multiply(${so.markup_pct} / 100)).format(${s.product.increment}, Math.floor) = ${price}`)
            ...
            if (!size) {
              ...
              //msg(`size = n(s.balance.currency).multiply(so.buy_pct).divide(100).divide(price).format('0.00000000')\nsize = n(${s.balance.currency}).multiply(${so.buy_pct}).divide(100).divide(${price}).format('0.00000000') = ${size}`)
            }
            msg(`(s.product.min_size && Number(size) >= Number(s.product.min_size)) || ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))\n(${s.product.min_size} && ${Number(size)} >= ${Number(s.product.min_size)}) || ('min_total' in s.product && ${s.product.min_total} && ${n(size).multiply(price).value()} >= ${Number(s.product.min_total)})`)
            ...
            //msg(`price = n(mid_price).add(n(mid_price).multiply(so.markup_pct / 100)).format(s.product.increment, Math.ceil)\nprice = n(${mid_price}).add(n(${mid_price}).multiply(${so.markup_pct} / 100)).format(${s.product.increment}, Math.ceil) = ${price}`)
            if (!size) {
              size = n(s.balance.asset).multiply(so.sell_pct / 100).format('0.00000000')
              //msg(`size = n(s.balance.asset).multiply(so.sell_pct / 100).format('0.00000000')\nsize = n(${s.balance.asset}).multiply(${so.sell_pct} / 100).format('0.00000000') = ${size}`)
            }
            msg(`(s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))\n(${s.product.min_size} && ${Number(size)} >= ${Number(s.product.min_size)}) || (${s.product.min_total} && ${n(size).multiply(price).value()} >= ${Number(s.product.min_total)})`)
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (!s.acted_on_stop && so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
